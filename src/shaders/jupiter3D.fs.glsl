...
uniform float uShininess = 10.0; // facteur de brillance
...
void main() {
    ...
    float specularIntensity = pow(max(dot(normal, h), 0.0), uShininess) * 0.15;

    // Combinaison de la texture de la terre et de la texture des nuages avec l'éclairage
    vec3 textureColor = texture(uJupiterTexture, vTexCoords).rgb;
